 ...
 * Baseline schema for WAJIDX platform on devaj database
 */

module.exports = {
  version: '20260830000001_001_initial_schema',
  name: 'Initial WAJIDX Database Schema',
  ...
  async down(query) {
    await query('DROP TABLE IF EXISTS wajidx_contact_messages');
    await query('DROP TABLE IF EXISTS wajidx_site_settings');
    await query('DROP TABLE IF EXISTS wajidx_project_images');
    await query('DROP TABLE IF EXISTS wajidx_project_features');
    await query('DROP TABLE IF EXISTS wajidx_project_technologies');
    await query('DROP TABLE IF EXISTS wajidx_projects');
    await query('DROP TABLE IF EXISTS wajidx_technologies');
    await query('DROP TABLE IF EXISTS wajidx_categories');
    await query('DROP TABLE IF EXISTS wajidx_admins');
  }
};
