...
async function seedDatabase() {
  ...
  try {
    console.log('[SEED] Running database schema migration on devaj database...');
    ...
    if (existingAdmins.length === 0) {
      ...
      console.log(`[SEED] Created default admin: ${adminEmail} (user: ${adminUsername})`);
    } else {
    ...
    for (const cat of categories) {
      const [existing] = await conn.query('SELECT id FROM wajidx_categories WHERE slug = ?', [cat.slug]);
    ...
    for (const tech of technologies) {
      const [existing] = await conn.query('SELECT id FROM wajidx_technologies WHERE slug = ?', [tech.slug]);
      if (existing.length === 0) {
        const [result] = await conn.query(
        ...
        techMap[tech.slug] = result.insertId;
      } else {
        techMap[tech.slug] = existing[0].id;
      }
    }
    console.log('[SEED] Technologies verified.');
    ...
    const projects = [
      {
        ...
        techSlugs: ['react', 'nodejs', 'mysql', 'tailwindcss', 'rest-api', 'docker'],
        features: [
      ...
      },
      {
        ...
        techSlugs: ['python-opencv', 'fastapi', 'nodejs', 'mysql', 'docker', 'rest-api'],
        features: [
      ...
      },
      {
        ...
        techSlugs: ['react', 'nodejs', 'mysql', 'tailwindcss', 'rest-api'],
        features: [
    ...
    for (const proj of projects) {
      const [existing] = await conn.query('SELECT id FROM wajidx_projects WHERE slug = ?', [proj.slug]);
      let projectId;
      if (existing.length === 0) {
        ...
        console.log(`[SEED] Created project: ${proj.title}`);
      } else {
        projectId = existing[0].id;
        console.log(`[SEED] Project already exists: ${proj.title}`);
      }

      // Link technologies
      await conn.query('DELETE FROM wajidx_project_technologies WHERE project_id = ?', [projectId]);
      for (const techSlug of proj.techSlugs) {
        const techId = techMap[techSlug];
        if (techId) {
          await conn.query('INSERT IGNORE INTO wajidx_project_technologies (project_id, technology_id) VALUES (?, ?)', [projectId, techId]);
        }
      }

      // Add features
      await conn.query('DELETE FROM wajidx_project_features WHERE project_id = ?', [projectId]);
      ...
      // Add images
      await conn.query('DELETE FROM wajidx_project_images WHERE project_id = ?', [projectId]);
    ...
    console.log('[SEED] Database seeding complete and ready!');
  } catch (error) {
